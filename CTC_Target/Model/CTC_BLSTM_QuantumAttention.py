...
class CTC_QuantumAttention(CTC_Multi_BLSTM):
    def __init__(self, trainData, trainLabel, trainSeqLength, featureShape, numClass, rnnLayers, hiddenNodules=128,
                 ...
                 occupyRate=-1):
        super(CTC_QuantumAttention, self).__init__(trainData=trainData, trainLabel=trainLabel,
    ...
    #         print('\rTesting %d/%d' % (startPosition, len(trainSeq)), end='')
    #         batchData = []
    ...
    #
    #         result = self.session.run(fetches=self.parameters['Logits_Reshape'],
    #                                   feed_dict={self.dataInput: batchData, self.seqLenInput: batachSeq})
    #         print(result)
    #         print(numpy.shape(result))
    #
    #         # result = self.session.run(fetches=self.parameters['Test'],
    #         #                           feed_dict={self.dataInput: batchData, self.seqLenInput: batachSeq})
    #         # print(result)
    #         # print(numpy.shape(result))
    #         exit()
...
if __name__ == '__main__':
    ...
    classifier = CTC_QuantumAttention(trainData=testData, trainLabel=testlabel, trainSeqLength=testSeq, featureShape=30,
