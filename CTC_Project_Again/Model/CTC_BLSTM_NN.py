...
class CTC_BLSTM_NN(CTC):
    def __init__(self, trainData, trainLabel, trainSeqLength, featureShape, numClass, hiddenNodules=128, rnnLayers=1,
                 ...
                 occupyRate=-1):
        super(CTC_BLSTM_NN, self).__init__(trainData=trainData, trainLabel=trainLabel, trainSeqLength=trainSeqLength,
    ...
    def BuildNetwork(self, learningRate):
        ...
        self.parameters['Layer1st_Reshape'] = tensorflow.reshape(tensor=self.dataInput, shape=[-1, self.featureShape],
                                                                 name='Layer1st_Reshape')
        self.parameters['Layer1st_FC'] = tensorflow.layers.dense(inputs=self.parameters['Layer1st_Reshape'], units=128,
                                                                 activation=tensorflow.nn.relu, name='Layer1st_FC')
        self.parameters['RNN_Input_Reshape'] = tensorflow.reshape(tensor=self.parameters['Layer1st_FC'],
                                                                  shape=[self.parameters['BatchSize'],
                                                                         self.parameters['TimeStep'], 128],
                                                                  name='RNN_Input_Reshape')
        ...
        (self.parameters['RNN_Output_Forward'], self.parameters['RNN_Output_Backward']), _ = \
            tensorflow.nn.bidirectional_dynamic_rnn(cell_fw=self.parameters['RNN_Cell_Forward'],
                                                    ...
                                                    inputs=self.parameters['RNN_Input_Reshape'],
        ...
        self.parameters['Layer3rd_FC'] = tensorflow.layers.dense(inputs=self.parameters['RNN_Reshape'], units=256,
                                                                 activation=tensorflow.nn.relu, name='Layer3rd_FC')
        self.parameters['Logits'] = tensorflow.layers.dense(inputs=self.parameters['Layer3rd_FC'], units=self.numClass,
                                                            activation=None)
