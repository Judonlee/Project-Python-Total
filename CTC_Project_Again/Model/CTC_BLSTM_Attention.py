...
class CTC_BLSTM(CTC):
    ...
    def BuildNetwork(self, learningRate):
        ...
        self.parameters['Attention_Weights'] = tensorflow.layers.dense(inputs=self.parameters['RNN_Reshape'],
                                                                       units=2 * self.hiddenNodules,
                                                                       activation=tensorflow.nn.softmax,
                                                                       name='Attention_Weights')
        self.parameters['Attention_Result'] = tensorflow.multiply(x=self.parameters['RNN_Reshape'],
                                                                  y=self.parameters['Attention_Weights'],
                                                                  name='Attention_Result')

        ###################################################################################################
        # Attention
        ###################################################################################################

        self.parameters['Logits'] = tensorflow.layers.dense(inputs=self.parameters['Attention_Result'],
                                                            units=self.numClass, activation=None, name='Logits')
