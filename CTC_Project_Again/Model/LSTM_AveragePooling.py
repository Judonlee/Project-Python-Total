...
class LSTM_AveragePooling(NeuralNetwork_Base):
    def __init__(self, trainData, trainLabel, trainSeqLength, featureShape, numClass, hiddenNodules=128, rnnLayers=1,
        ...
        super(LSTM_AveragePooling, self).__init__(trainData=trainData, trainLabel=trainLabel, batchSize=batchSize,
    ...
    def BuildNetwork(self, learningRate):
        ...
        self.appendMatrix = tensorflow.placeholder(dtype=tensorflow.float32, shape=[None, self.hiddenNodules],
                                                   name='appendMatrix')
        ...
        self.parameters['RNN_ReduceSum'] = tensorflow.reduce_sum(input_tensor=self.parameters['RNN_Outputs'], axis=1,
                                                                 name='RNN_ReduceSum')
        self.parameters['RNN_Average'] = self.parameters['RNN_ReduceSum'] * self.appendMatrix

        self.parameters['Logits'] = tensorflow.layers.dense(inputs=self.parameters['RNN_Average'],
        ...
        while startPosition < len(trainData):
            ...
            loss, _ = self.session.run(fetches=[self.loss, self.train],
                                       feed_dict={self.dataInput: batchTrainData, self.labelInput: batchTrainLabel,
                                                  ...
                                                  self.appendMatrix: self.AppendMatrixGeneration(batchTrainSeqLen)})
    ...
    def AppendMatrixGeneration(self, seqInput):
        appendMatrix = []
        for sample in seqInput:
            currentData = numpy.ones(self.hiddenNodules) / sample
            appendMatrix.append(currentData)
        return appendMatrix
