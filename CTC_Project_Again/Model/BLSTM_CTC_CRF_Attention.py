...
class BLSTM_CTC_CRF_Attention(CTC):
    def __init__(self, trainData, trainLabel, trainSeqLength, featureShape, numClass, hiddenNodules=128, rnnLayers=1,
                 ...
                 occupyRate=-1):
        super(BLSTM_CTC_CRF_Attention, self).__init__(trainData=trainData, trainLabel=trainLabel,
                                                      trainSeqLength=trainSeqLength, featureShape=featureShape,
                                                      batchSize=batchSize, numClass=numClass,
                                                      hiddenNodules=hiddenNodules, rnnLayers=rnnLayers,
    ...
    def BuildNetwork(self, learningRate):
        ...
        self.parameters['CRF_AttentionWeights'] = tensorflow.layers.dense(inputs=self.parameters['RNN_Reshape'],
                                                                          units=self.hiddenNodules * 2,
                                                                          activation=tensorflow.nn.softmax,
                                                                          name='CRF_AttentionWeights')
        self.parameters['CRF_AfterAttention'] = tensorflow.multiply(x=self.parameters['RNN_Reshape'],
                                                                    y=self.parameters['CRF_AttentionWeights'],
                                                                    name='CRF_AfterAttention')

        self.parameters['CRF_Logits'] = tensorflow.layers.dense(inputs=self.parameters['CRF_AfterAttention'],
