    ...
    for bands in [30]:
        for session in range(1, 6):
            for gender in ['Female', 'Male']:
                ...
                netpath = 'D:/ProjectData/BrandNewCTC/SingleBLSTM-Choosed/Bands-%d-Session-%d-%s/' % (
                    bands, session, gender)
                savepath = 'Single_BLSTM_CTC_CRF_%s/Bands-%d-Session-%d-%s/' % (used, bands, session, gender)
