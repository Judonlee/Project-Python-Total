    ...
    for bands in [30]:
        for session in range(1, 6):
            for gender in ['Female', 'Male']:
                ...
                netpath = 'D:/ProjectData/BrandNewCTC/Double_BLSTM_CTC_CRF_%s/Bands-%d-Session-%d-%s/' % (
                    part, bands, session, gender)

                for testGender in ['Female', 'Male']:
                    savepath = 'Result_Double_BLSTM_CTC_CRF_%s/Bands-%d-Session-%d-%s-%s/' % (
                    ...
                    for episode in range(100):
                        ...
                        with graph.as_default():
                            classifier = BLSTM_CTC_CRF(trainData=None, trainLabel=None, trainSeqLength=None,
                                                       ...
                                                       batchSize=32, startFlag=False, rnnLayers=2)
