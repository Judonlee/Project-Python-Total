    ...
    for bands in [30, 40, 60, 80, 100, 120]:
        for appoint in range(10):
            trainData, trainLabel, trainSeq, trainScription, testData, testLabel, testSeq, testScription = \
                IEMOCAP_Loader_Npy(
                    loadpath='/mnt/external/Bobs/Npy-TotalWrapper/Bands-%d-%d/' % (bands, appoint))

            savepath = '/home/bztbztbzt/Records-Result-CTC-LR1e-3-RMSP-Restart/Bands-%d-%d/' \
                       % (bands, appoint)
            ...
            for trace in range(100):
                ...
                with graph.as_default():
                    classifier = CTC_BLSTM(trainData=trainData, trainLabel=trainScription, trainSeqLength=trainSeq,
                                           ...
                                           batchSize=64)
                    classifier.Load(
                        '/mnt/external/Bobs/Records-CTC-Class5-LR1E-3-RMSP/Bands-%d-%d/%04d-Network'
                        % (bands, appoint, trace))
