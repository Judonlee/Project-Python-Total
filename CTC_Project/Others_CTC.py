...
def fake_data(num_examples, num_features, num_labels, min_size=10, max_size=15):
    ...
    labels = np.asarray(
        [np.random.randint(0, num_labels, np.random.randint(1, inputs[i].shape[0], (1,))).astype(np.int64) for i, _ in
         enumerate(timesteps)])
    #labels = [[1, 2, 2], [2, 1, 1]]
...
with graph.as_default():
    ...
    for _ in range(num_layers):
        cell = tf.contrib.rnn.LSTMCell(num_hidden)  # Or LSTMCell(num_units)
        cells.append(cell)
...
with tf.Session(graph=graph) as session:
    ...
    d = session.run(logits, feed_dict=feed)
    for indexX in range(len(d)):
        for indexY in range(len(d[indexX])):
            print(np.argmax(np.array(d[indexX][indexY])), end=',')
        print()
