...
class LSTM_FinalPooling(NeuralNetwork_Base):
    ...
    def Test(self, testData, testLabel, testSequence, savename='None'):
        matrix = numpy.zeros((len(testLabel[0]), len(testLabel[0])))
        ...
        while startPosition < len(testData):
            ...
            predict = self.session.run(fetches=self.parameters['PredictProbability'], feed_dict={
        ...
        if savename != 'None':
            ...
            for index in range(len(testLabel)):
                file.write(str(numpy.argmax(numpy.array(testLabel[index]))))
